	...
	public static void main(String[] args) {
		...
		while (currentTime < 20) {
			for (int i = 1; i < lines.size() - 1; i++) {
				...
				System.out.println(lines.size() + " " + i + " " + currentTime);
				String[] strPart = lines.get(i).split(" ");
				String[] nextStrPart = lines.get(i+1).split(" ");
				System.out.println(Integer.parseInt(nextStrPart[1]));
				// Line can begin running at its specified arrival time
				while (Integer.parseInt(nextStrPart[1]) > currentTime) {
				
					if (Integer.parseInt(strPart[1]) == currentTime) {
						int tempCurrentTime;
						int tempJobNum;
						int tempMem;
						int tempDev;
						...
						case "A":
							...
							// Add to hold queue (we're adding everything then
							// taking first elements to run)
						...
						case "Q":
							...
							if (sys.gethSJF().contains(sys.getAllJobs().get(tempJobNum - 1))
								...
								// Remove devices from system if available or
								// put job in wait queue
						...
						case "L":
							...
							if (sys.gethSJF().contains(sys.getAllJobs().get(tempJobNum - 1))
									|| sys.gethFIFO().contains(sys.getAllJobs().get(tempJobNum - 1))) {
								// Decrease required # of devices for that job
								// (if not yet running)
								sys.getAllJobs().get(tempJobNum - 1).decNumDev(numDev);
							} else if (sys.getrQueue().contains(sys.getAllJobs().get(tempJobNum - 1))) {
								// Add back devices to system and decrease from
								// job
								sys.incaDev(tempDev);
						...
						if (!sys.gethSJF().isEmpty() && sys.gethSJF().peek().getMem() <= sys.getaMem()
								&& sys.gethSJF().peek().getNumDev() <= sys.getaDev()) {
							...
							sys.gethSJF().remove();
						} else if (sys.gethFIFO().isEmpty() && sys.gethFIFO().peek().getMem() <= sys.getaMem()
								&& sys.gethFIFO().peek().getNumDev() <= sys.getaDev()) {
							// Add first position of Hold Queue to ready queue
						...
						if (quantumCount < 4) {
							if (runningJob.getrT() > 0) {
								...
								System.out.println(
										"rT decreased " + runningJob.getrT() + " -> " + (runningJob.getrT() - 1));
								// Decrease rT of job
								runningJob.decrT();
								quantumCount++;
							}
					...
					currentTime++;
					System.out.println("System Mem=" + totMem + ", Dev=" + numDev + ", QTime=" + qTime
							+ ", CurrentTime=" + currentTime);
				}
			}
		}
